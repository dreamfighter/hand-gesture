...
{
    public partial class RecognitionV2 : Form
    {
        ...
        private static int MIN_HAND_RADIUS = 25;
        ...
        private void settingCapture()
        {
            ...
            cap = new Capture("video_noise/realtime.avi");
        ...
        private String action(String gesture)
        {
            ...
            switch (gesture)
            {
                ...
                case "C": return "Swipe Right";
                case "D": return "Swipe Left";
                case "E": return "Grap";
        ...
        private void timerObserve_Tick(object sender, EventArgs e)
        {
            ...
            */
            if (handShape == null || handShape.maximumInscribedCircle.Radius < MIN_HAND_RADIUS)
            {
                gesture = action("X");
                return;
            }
            Console.WriteLine("radius " + handShape.maximumInscribedCircle.Radius);

            handStatus = svm.predict(handShape);
            Console.WriteLine(handStatus);
            //observation.Add(handStatus);
            queue.Enqueue(handStatus);

            if (queue.Count > MAX_STACK)
            {
                queue.Dequeue();
            }
            ...
            double prob = 0;
            String tempGesture = hmm.predict(queue.ToArray(), out prob);
            if (tempGesture != "X")
            {
                queue.Clear();
            }
            gesture = action(tempGesture);
            /*
            if (prob >= hmm.model)
            {
                gesture = action(tempGesture);
        ...
        {
            RecordVideo form = new RecordVideo(ListOfDevices.SelectedIndex);
            form.Show();
        }
