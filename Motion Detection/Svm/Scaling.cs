...
namespace SVM
{
    ...
    public static class Scaling
    {
        ...
        /// Scales a problem using the provided range.  This will not affect the parameter.
        /// </summary>
        /// <param name="node">The Node of Attribute</param>
        /// <param name="range">The Range transform to use in scaling</param>
        /// <returns>The Scaled problem</returns>
        public static Node[] Scale(this IRangeTransform range, Node[] node)
        {
            for (int j = 0; j < node.Length; j++)
                node[j] = new Node(node[j].Index, range.Transform(node[j].Value, node[j].Index));
            return node;
        }
    }
}
